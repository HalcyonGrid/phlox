...
namespace Halcyon.Phlox.Glue
{
    public interface ISystemAPI
    {
        ...
        LSLList halParseString2List(string src, LSLList separators, LSLList spacers, LSLList args);
        ...
        void halRezObject(string inventory, Vector3 pos, Vector3 vel, Quaternion rot, int param);
        void iwRezObject(string inventory, Vector3 pos, Vector3 vel, Quaternion rot, int param);
        void halRezAtRoot(string inventory, Vector3 pos, Vector3 vel, Quaternion rot, int param);
        void iwRezAtRoot(string inventory, Vector3 pos, Vector3 vel, Quaternion rot, int param);
        string halRezPrim(LSLList primParams, LSLList particleSystem, LSLList inventory, Vector3 pos, Vector3 vel, Quaternion rot, int param);
        ...
        LSLList halGetWorldBoundingBox(string obj);
        ...
        int halSubStringIndex(string source, string pattern, int offset, int isCaseSensitive);
        ...
        void halSetWind(int type, Vector3 offset, Vector3 speed);
        void iwSetWind(int type, Vector3 offset, Vector3 speed);
        Vector3 halWind(Vector3 offset);
        Vector3 iwWind(Vector3 offset);
        int halHasParcelPowers(int groupPower);
        int iwHasParcelPowers(int groupPower);
        Vector3 halGroundSurfaceNormal(Vector3 offset);
        Vector3 iwGroundSurfaceNormal(Vector3 offset);
        string halRequestAnimationData(string name);
        ...
        int halGetLocalTime();
        int iwGetLocalTime();
        int halGetLocalTimeOffset();
        int iwGetLocalTimeOffset();
        string halFormatTime(int unixtime, int isUTC, string format);
        string iwFormatTime(int unixtime, int isUTC, string format);
        int halCheckRezError(Vector3 pos, int isTemp, int landImpact);
        ...
        int halChar2Int(string src, int index);
        int iwChar2Int(string src, int index);
        string halInt2Char(int num);
        string iwInt2Char(int num);
        string halReplaceString(string str, string pattern, string replacement);
        string iwReplaceString(string str, string pattern, string replacement);
        string halFormatString(string str, LSLList values);
        string iwFormatString(string str, LSLList values);
        int halMatchString(string str, string pattern, int matchType);
        int iwMatchString(string str, string pattern, int matchType);
        string halStringCodec(string str, string pattern, int operation, LSLList extraParams);
        string iwStringCodec(string str, string pattern, int operation, LSLList extraParams);
        int halMatchList(LSLList list1, LSLList list2, int matchType);
        int iwMatchList(LSLList list1, LSLList list2, int matchType);
        Vector3 halColorConvert(Vector3 input, int color1, int color2);
        Vector3 iwColorConvert(Vector3 input, int color1, int color2);
        Vector3 halNameToColor(string name);
        Vector3 iwNameToColor(string name);
        int halVerifyType(string str, int type);
        int iwVerifyType(string str, int type);
        int halGroupInvite(string group, string user, string role);
        int iwGroupInvite(string group, string user, string role);
        int halGroupEject(string group, string user);
        int iwGroupEject(string group, string user);
        string halGetAgentData(string id, int data);
        string iwGetAgentData(string id, int data);
        int halIsPlusUser(string id);
        int iwIsPlusUser(string id);
        void llAttachToAvatarTemp(int attachPoint);
        int halListIncludesElements(LSLList src, LSLList elements, int any);
        ...
        LSLList halReverseList(LSLList src, int stride);
        LSLList iwReverseList(LSLList src, int stride);
        LSLList halSearchInventory(int type, string pattern, int matchtype);
        LSLList iwSearchInventory(int type, string pattern, int matchtype);
        LSLList halSearchLinkInventory(int link, int type, string pattern, int matchtype);
        LSLList iwSearchLinkInventory(int link, int type, string pattern, int matchtype);
        int halIntRand(int max);
        int iwIntRand(int max);
        int halIntRandRange(int min, int max);
        int iwIntRandRange(int min, int max);
        float halFrandRange(float min, float max);
        ...
        LSLList halListRemoveElements(LSLList src, LSLList elements, int count, int mode);
        LSLList iwListRemoveElements(LSLList src, LSLList elements, int count, int mode);
        LSLList halListRemoveDuplicates(LSLList src);
        LSLList iwListRemoveDuplicates(LSLList src);
        void halStartLinkAnimation(int link, string anim);
        void iwStartLinkAnimation(int link, string anim);
        void halStopLinkAnimation(int link, string anim);
        void iwStopLinkAnimation(int link, string anim);
        int halClampInt(int value, int min, int max);
        ...
        LSLList halSearchLinksByName(string pattern, int matchType, int linksOnly);
        LSLList iwSearchLinksByName(string pattern, int matchType, int linksOnly);
        LSLList halSearchLinksByDesc(string pattern, int matchType, int linksOnly);
        ...
        int halValidateURL(string url);
        int iwValidateURL(string url);
        int halRemoteLoadScriptPin(string target,string name,int pin,int running,int start_param);
        int iwRemoteLoadScriptPin(string target,string name,int pin,int running,int start_param);
        void halDeliverInventory(int linknumber, string destination, string inventory);
        void iwDeliverInventory(int linknumber, string destination, string inventory);
        void halDeliverInventoryList(int linknumber, string target, string folder, LSLList inventory);
        ...
        string halGiveMoney(string destination, int amount);
        string iwGiveMoney(string destination, int amount);
        void halStandTarget(Vector3 offset, Quaternion rot);
        void iwStandTarget(Vector3 offset, Quaternion rot);
        void halLinkStandTarget(int link, Vector3 offset, Quaternion rot);
